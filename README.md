...
​The Problem
​Living with RA means dealing with unpredictable pain, severe fatigue, and sudden flare-ups. Traditional health advice is often static and fails to account for good days versus bad days.

​Rheumio solves this by:
​Providing instant, personalized guidance.
​Remembering patient history, including pain scores and energy levels.
​Filtering advice through strict safety guardrails, such as preventing heavy exercise during high-fatigue Red days.
...
​Persona and Guardrails: A deep system instruction set ensures an empathetic tone and enforces medical safety rules.

​The Tool Layer: Custom Python functions that allow the agent to use specific capabilities:
...
​State Management: Session-based Python 
...
​Input: User says, I am feeling very tired today, but I wanted to go for a run.

​Decision: Gemini recognizes the tired intent and calls update_energy_meter(state=Red).

​Execution: The system updates the internal 
state to Red.

​Feedback: Gemini receives confirmation of the state change.

​Response: Based on safety guardrails for Red days, Rheumio responds: Since your energy is low today, I strongly recommend skipping the run. Let us focus on gentle stretching or a 10-minute meditation instead.
